...
const CriteriaArray = ({
  ...
  return (
    <FormGroup row={false}>
      <Label>{t("fund.criteria")}</Label>

      <FormText>
        <TranslatedHtml content={"form.fund.criteria.help"} />
      </FormText>
      ...
      {values && values.length > 0 && values.map((value, index) => (
        <div key={index}>
          <Field component={FormikArrayInput}
            name={`${name}.${index}`}
            placeholder="form.fund.criteria.placeholder"
      ...
      <Button onClick={() => push("")} className="btn btn-sm d-block">
        <Icon name="plus" size={20} color="white" /> {t("form.fund.addCriterion")}
      </Button>
