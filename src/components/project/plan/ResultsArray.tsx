...
const ResultsArray: React.FC<IProps> = ({ push, form, remove, name, }: IProps) => {
  ...
  return (
    <FormGroup row={false}>
      <Label>{t("project.results")}</Label>

      <FormText>
        <TranslatedHtml content={"form.project.results.help"} />
      </FormText>
      ...
      {values && values.length > 0 && values.map((value, index: number) => (
        <div key={index}>
          <Field component={FormikArrayInput}
            name={`${name}.${index}`}
            placeholder="form.project.results.placeholder"
      ...
      <Button onClick={() => push("")} className="btn btn-sm d-block">
        <Icon name="plus" size={20} /> {t("form.project.plan.addResult")}
      </Button>
