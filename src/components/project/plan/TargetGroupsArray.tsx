...
const TargetGroupsArray: React.FC<IProps> = ({ push, form, remove, name, }: IProps) => {
  ...
  return (
    <FormGroup row={false}>
      <Label>{t("project.targetGroups")}</Label>

      <FormText>
        <TranslatedHtml content={"form.project.targetGroups.help"} />
      </FormText>
      ...
      {values && values.length > 0 && values.map((value, index: number) => (
        <div key={index}>
          <Field component={FormikArrayInput}
            name={`${name}.${index}`}
            placeholder="form.project.targetGroups.placeholder"
      ...
      <Button onClick={() => push("")} className="btn btn-sm d-block">
        <Icon name="plus" size={20} /> {t("form.project.plan.addTargetGroup")}
      </Button>
...
export default TargetGroupsArray
